<?php
    ...
    function check_name($name, $conn){ // -1=invalid and 0=notunique 1=correct
        ...
        $sql = "SELECT `GName` from `Game` where `GName` = '".$name."'";
    ...
    */
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if(check_name($gname, $conn) == -1){
            echo "Invalid game name format!";
        }else if(check_name($gname, $conn) == 0){
            echo "Our site already has that game. Publish new games!";
            ...
            $sql = "INSERT INTO Game(GName, GPoster, GGenre, GRate, GReleasedDate, GPublisher, GTrailer) VALUES('$gname', '$filename', '$ggenre', $formated_rate, '$formated_date', '$gpublisher', '$formated_url');";
            if(mysqli_query($conn, $sql)){
                $username = $_SESSION['username'];
                $gid = mysqli_insert_id($conn);
                $sql = "INSERT INTO GamesPublishersHistory(UserName, GID) VALUES('$username', $gid);";
                if(mysqli_query($conn, $sql)){
                    echo "Game has been published successfully! Now users can enjoy playing it";
                }else{
